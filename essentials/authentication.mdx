...
```
POST https://api.predictus.inf.br/auth
```
...
```bash
curl --location 'https://api.predictus.inf.br/auth' \
...
- **Não há refresh token** neste momento
...
| `401`  | Token expirado ou ausente                 | Gere novo token e reenvie         |
